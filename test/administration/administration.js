document.addEventListener("DOMContentLoaded", () => {
  ...
  function fetchCSV(level) {
    let csvFile;
    if (level === "county") csvFile = "county.csv"; // 县级数据文件
    else if (level === "prefecture") csvFile = "prefecture.csv"; // 市级数据文件
        ...
        function checkInput() {
          ...
          if (userInput !== "" && aliases.includes(userInput)) {
            ...
            hintBtn.style.display =
              hintText.style.display === "none" ? "inline-block" : "none"; // 显示或隐藏提示按钮
          }
        }
      });
  }
